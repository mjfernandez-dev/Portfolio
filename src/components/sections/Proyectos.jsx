...
export default function Proyectos({ isDarkMode }) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6" role="region" aria-labelledby="proyectos-heading" ref={ref}>
      ...
      >
        <Briefcase className={`w-7 h-7 sm:w-8 sm:h-8 ${t(isDarkMode, 'text-indigo-400', 'text-indigo-500')}`} aria-hidden="true" />
      ...
      <motion.div
        ...
        className={`backdrop-blur-sm rounded-xl p-4 sm:p-6 border mt-6 sm:mt-8 ${t(isDarkMode,
      ...
      >
        <p className={`text-center text-sm sm:text-base ${t(isDarkMode, 'text-slate-300', 'text-slate-600')}`}>
