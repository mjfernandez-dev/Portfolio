...
function GlowCard({ isDarkMode, children, className = '', delay = 0, isInView }) {
  return (
    ...
    >
      ...
      <div className={`relative backdrop-blur-sm rounded-xl border transition-all hover:scale-105 ${t(isDarkMode,
        'bg-white/[0.04] border-white/10 hover:border-indigo-500/40 hover:shadow-[0_0_24px_rgba(99,102,241,0.2)]',
        'bg-white border-slate-200 hover:border-indigo-300/70 hover:shadow-[0_4px_24px_rgba(99,102,241,0.12)]'
      )}`}>
        {children}
      </div>
    </motion.div>
...
export default function Habilidades({ isDarkMode }) {
  ...
  return (
    <div className="space-y-8 sm:space-y-10" role="region" aria-labelledby="habilidades-heading" ref={ref}>
      ...
      >
        <Code2 className={`w-7 h-7 sm:w-8 sm:h-8 ${t(isDarkMode, 'text-indigo-400', 'text-indigo-500')}`} aria-hidden="true" />
      ...
      <div>
        <motion.h4
          ...
          className={`text-lg sm:text-xl font-semibold mb-4 ${t(isDarkMode, 'text-indigo-300', 'text-indigo-600')}`}
        >
                ...
                <div className="flex flex-wrap gap-2">
                  {lang.tools.map((tool, i) => (
                    <motion.span
                      ...
                      className={`px-2.5 sm:px-3 py-1 rounded-full text-xs sm:text-sm cursor-default border ${t(isDarkMode,
                        'bg-indigo-900/40 text-indigo-200 border-indigo-700/30',
          ...
          {skillsData.devtools.map((tool, index) => (
            <GlowCard key={index} isDarkMode={isDarkMode} delay={0.2 + index * 0.06} isInView={isInView}>
              <div className="p-4 text-center">
                ...
                <p className={`text-xs sm:text-sm font-medium ${t(isDarkMode, 'text-slate-300', 'text-slate-600')}`}>{tool.name}</p>
              </div>
            </GlowCard>
          ))}
        </div>
      ...
      <div>
        <motion.h4
          ...
          className={`text-lg sm:text-xl font-semibold mb-4 ${t(isDarkMode, 'text-blue-300', 'text-blue-600')}`}
        >
          Bases de Datos
        </motion.h4>
